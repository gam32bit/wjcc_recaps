#!/usr/bin/env python3
"""Draft newsletter prose for the top-ranked agenda items (PREVIEW only).

The recap — the default product — no longer drafts prose at all: it quotes the
agenda's own wording and counts what can be counted (see render.py and
pubcomment.py). This module serves the `--preview` product only.
